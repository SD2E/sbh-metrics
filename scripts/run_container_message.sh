...
DOCKER_ENVS=$(python3 ${DIR}/secrets_to_docker_envs.py ${REACTOR_SECRETS_FILE})
# Set the Reactor.local flag. Also ensures DOCKER_ENVS is not empty
DOCKER_ENVS="-e LOCALONLY=1 ${DOCKER_ENVS} -e SBH_TEST=${2}"
...
if [ ! -d "${AGAVE_CREDS}" ]; then
    ...
    if ((AGAVE_PREFER_PYTHON)); then
      ...
      eval python3 ${DIR}/refresh_agave_credentials.py
    else
